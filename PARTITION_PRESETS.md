...
![What a preset describes](img/presets-shape.svg)
...
proportions and prints what it came up with. For the same groups with
`row_count=10000`:

```
info: init_partition_row_distribution_preset: preset_name=main, total_partitions=647, \
total_rows=10000, partitions/rows -> 453(~70.015%):10, 129(~19.93%):25, 64(~9.89%):35, 1(~0.15%):5
```
...
extra partition of a leftover size. The `1(~0.15%):5` group above is such a
partition: it carries the 5 rows the 3 requested groups could not fit. It shows
up as an additional group in the printed line, and the workload does not need to
care about it.
...
![Consecutive indexes land on different partitions](img/presets-row-addressing.svg)
...
let partition = db.get_partition_info("main", idx).await;
let pk = hash(partition.idx);
// rows start expiring by TTL after 100 seconds of the run
...
**Fewer, cheaper loaders.** Writing N rows in one request instead of N requests
means the loader does the per-request work once instead of N times: one rune
function call, one statement dispatch, one round trip, one latency sample. The
number of requests needed to populate a data set drops by the average number of
rows per batch, and so does the client side CPU and the number of in-flight
requests needed to keep the cluster busy.

In practice that is what decides how big the stress VMs have to be. A populating
run that needed several loader instances to saturate a cluster can often be done
by fewer or by smaller ones, simply because most of the cost per row was the
request around it, not the row itself.

**How much** depends only on how many rows end up in one batch:

| rows per partition | `batch_size` | requests for 1M rows | reduction |
|---|---|---|---|
| 5 | 2 | 600 000 | 1.7x |
| 5 | 5 | 200 000 | 5x |
| 100 | 20 | 50 000 | 20x |
...
The upper bound is the partition size itself - a batch never spans two
partitions, so a data set of small partitions has less to gain than one of fat
partitions. Read the reduction off the `total_batches` value printed at startup:
it is `row_count / total_batches`.

**And the cluster does less work too.** Latte sends `LOGGED` batches. ScyllaDB
and Cassandra skip the batchlog when every statement of a `LOGGED` batch targets
the **same** partition, because such a batch is already atomic. A batch spanning
several partitions makes the coordinator durably write the batch to two batchlog
replicas before anything is applied.

Since a preset deliberately maps consecutive stress iterations to *different*
partitions, a batch built from consecutive iteration indexes would hit exactly
that case. The functions below give a batch whose rows all belong to one
partition, while keeping the partition size proportions the preset provides.

![A naive batch spans partitions, a preset built batch does not](img/presets-single-partition-batch.svg)
...
It prints the number of batches needed to write every row exactly once. For a
10000 row preset of `"50:4,50:6"` partitions with `batch_size=2`:
...
The stress iteration index enumerates *batches*, not rows. Every index first
picks a partition size group, proportionally to the number of batches that group
holds, and inside a group the partitions are walked one by one before the next
batch of each of them follows. Consecutive batches therefore keep landing on
different partitions.

The diagram below uses a preset of a single partition size group, so that only
the walk inside a group is visible:

![How the stress iteration index walks the batches](img/presets-batch-walk.svg)
...
The partition size groups are picked proportionally to the number of batches
they hold, so **any** prefix of a pass keeps the size proportions of the preset,
within a single batch of the exact share. A run which is cut short - a time
boxed one, or an interrupted populate - therefore still covers the whole
partition size mix rather than the biggest partitions only, and the reported
latency does not step as the run moves from one group to another.
...
| time boxed throughput run | e.g. `30m` | correct data, a proportional part of every partition size group |
| longer than needed | `> total_batches` | rows get rewritten, data stays correct |
| shorter than needed | `< total_batches` | a proportional part of every partition size group gets written |

A shorter run is not a single partition size group, it is a proportional sample
of all of them, so the measured numbers describe the partition size mix the
preset asks for rather than its biggest partitions.

A too small value is still the most common mistake when the goal is to populate.
It does not report an error. Some partitions end up complete, some hold a part
of their rows and most are not written at all, which the reading validation
...
On the CQL side keep `batch_size` small enough to stay below the ScyllaDB
...
On the Alternator side the limit is a hard one: a `BatchWriteItem` request takes
at most **25** items, and `db.batch_write_item` neither splits nor rejects a
longer list. So an Alternator workload must either keep `batch_size` at `25` or
below, or slice the rows of a batch into chunks of 25 itself.
