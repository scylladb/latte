...
impl Context {
    ...
    pub async fn execute_with_result(&self, cql: &str) -> Result<Value, CassError> {
        self._execute_with_result(Some(cql), None, None).await
    }
    ...
    pub async fn execute_prepared_with_result(
        ...
        self._execute_with_result(None, Some(key), Some(params))
            .await
    }

    async fn _execute_with_result(
        &self,
        cql: Option<&str>,
        key: Option<&str>,
        params: Option<Value>,
        ...
        while current_attempt_num <= self.retry_number {
            ...
            if let Ok(rows_result) = page.clone().into_rows_result() {
                let column_specs = rows_result.column_specs();
                let row_iterator = rows_result.rows::<Row>()?;
