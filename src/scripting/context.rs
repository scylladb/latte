...
fn cql_value_to_rune_value(value: &CqlValue) -> Result<Value, Box<CassError>> {
    match value {
        ...
        CqlValue::Empty => Ok(Value::EmptyTuple),
...
impl Context {
    ...
    pub async fn signal_failure(&self, message: &str) -> Result<(), CassError> {
        let err = CassError(CassErrorKind::Error(message.to_string()));
        let current_attempt_num = 0;
        if self.validation_strategy == ValidationStrategy::Retry {
            // NOTE: we are out of the retry-loop here, so behave as "fail-fast"
            Err(err) // stop stress execution
        } else if self.validation_strategy == ValidationStrategy::FailFast {
            Err(err) // stop stress execution
        } else if self.validation_strategy == ValidationStrategy::Ignore {
            handle_retry_error(self, current_attempt_num, err).await;
            Ok(()) // handle/print error and go on.
        } else {
                        ...
                        Ok(row) => {
                            for (index, cql_value) in row.columns.iter().enumerate() {
                                ...
                                current_row_data.push((
                                    col_name.to_string(),
                                    cql_value.clone().expect("failed to get column name"),
                                ));
                            }
                        }
                        Err(_) => {
