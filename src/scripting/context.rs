                    ...
                    && idx < cycle_type_1.n_rows_for_left + done_cycle_type_1_rows
                {
                    let ret = partn_offset
                        + (idx - done_cycle_type_1_rows
                            ...
                            % current_partn_count;
                    return (ret, current_partn.partition_size);
                }
            } else {
...
impl Context {
    ...
    pub async fn get_partition(
        ...
        Ok(preset.get_partition(idx).await)
    }
    ...
    pub async fn execute_with_validation(
        ...
        if expected_rows_num_min > expected_rows_num_max {
            return Err(CassError(CassErrorKind::Error(
                "Expected 'minimum' of rows number cannot be less than 'maximum'".to_string(),
            )));
        }
        self._execute(
            Some(cql),
...
mod tests {
        ...
        for (rows_per_partitions_base, rows_per_partitions_groups) in rows_per_partitions_base_and_groups_mapping {
            ...
            for (idx, expected_partition_idx) in expected_idx_partition_idx_mapping.clone() {
                let (p_idx, _p_size) = tokio::runtime::Runtime::new().unwrap().block_on(async {
                    ctxt.get_partition(preset_name, idx).await
    ...
    fn test_partition_row_distribution_preset_01_pos_single_group_evenly_divisible() {
        ...
        init_and_use_partition_row_distribution_preset(
            ...
            vec![PartitionGroup{ n_rows_per_group: 1000, n_partitions: 40, partition_size: 25}],
            vec![
    ...
    fn test_partition_row_distribution_preset_02_pos_single_group_unevenly_divisible() {
        ...
        init_and_use_partition_row_distribution_preset(
            ...
            vec![
                PartitionGroup{ n_rows_per_group: 988, n_partitions: 76, partition_size: 13},
                PartitionGroup{ n_rows_per_group: 12, n_partitions: 1, partition_size: 12},
            ],
            vec![
    ...
    fn test_partition_row_distribution_preset_03_pos_multiple_groups_with_implicit_one() {
        ...
        init_and_use_partition_row_distribution_preset(
            ...
            vec![
                PartitionGroup{ n_rows_per_group: 408, n_partitions: 17, partition_size: 24},
                PartitionGroup{ n_rows_per_group: 312, n_partitions: 26, partition_size: 12},
                PartitionGroup{ n_rows_per_group: 276, n_partitions: 46, partition_size: 6},
                PartitionGroup{ n_rows_per_group: 4, n_partitions: 1, partition_size: 4},
            ],
            vec![
    ...
    fn test_partition_row_distribution_preset_04_pos_multiple_groups_without_implicit_one() {
        ...
        init_and_use_partition_row_distribution_preset(
            ...
            vec![
                PartitionGroup{ n_rows_per_group: 6640, n_partitions: 332, partition_size: 20},
                PartitionGroup{ n_rows_per_group: 3310, n_partitions: 331, partition_size: 10},
                PartitionGroup{ n_rows_per_group: 50, n_partitions: 1, partition_size: 50},
            ],
            vec![
