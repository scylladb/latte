...
impl RowDistributionPreset {
    ...
    pub fn generate_row_distributions(&mut self) {
        let mut other_rows: u64 = self.partition_groups.iter().map(|pg| pg.n_rows_per_group).sum();
    ...
    pub async fn get_partition_idx(&self, idx: u64) -> u64 {
        self._get_partition_idx(idx, 0, self.partition_groups.clone(), self.row_distributions.clone()).await
    }
